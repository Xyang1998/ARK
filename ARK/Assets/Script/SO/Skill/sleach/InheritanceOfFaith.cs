...
public class InheritanceOfFaith : BaseSkill
{
    ...
    {
        _drop = Instantiate(drop,target.CharacterGO.transform);
        _drop.AddComponent<LookAtCamera>();
        Drop(initiator,target,damage,critical,isFirst,isFinal,isDamage,initiatorNP,targetNP,isMainTarget).Forget();
    }
